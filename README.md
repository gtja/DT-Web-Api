...
- 请求 `videoId=live` 时，从 `capacity` 中选择当前设备 `active=true` 且有可用播放地址的通道，
  不从 `liveStatus` 的可选镜头目录猜测。飞机首先跟随 OSD `live_status` 推断的当前通道，无明确通道时优先选择广角；广角未开流或没有可拉取地址时，
  再按真实通道 ID 顺序尝试其他已开流通道。机场仍按真实通道 ID 排序选择第一路。
...
飞机 `START live` 后，每 2 秒合并处理 OSD 变化，并在同一直播执行线程上切换 FFmpeg 输入，沿用原媒体推流地址。
仅接收配置绑定的飞机/机巢消息，忽略中继基站和乱序状态。`status=1` 且 `error_status=0` 表示可用：
唯一可用通道、或相邻快照中唯一新增/`video_type` 变化的可用通道作为跟随依据；重复消息、质量和带宽变化不切换。
多路同时开启且无明确变化时，保留之前推断的可用通道；首次或无法确定时广角兜底，再尝试其他可用流。
这是根据状态变化推断，接口没有唯一的“界面选中镜头”标记；如果用户切换界面但 OSD 完全不变，则无法识别该切换。
新通道暂无播放地址时使用可用兜底并继续重试；查询失败保留当前转推，FFmpeg 切换失败尝试恢复原流。
STOP 后取消跟随；明确传完整 videoId 的请求不会启用自动跟随。切换需要重启转推进程，可能短暂中断，时长取决于实际网络和播放器缓冲。
...
3. 机场或明确通道已正常转推时直接复用；飞机 `live` 在复用前重新查询 SkyCC `capacity` 确认当前优先通道。
   其余情况按上述规则选择通道，
...
道通目标通道必须已经有直播源；没有任何可用源时返回明确错误，需要先在天穹开启直播。
不会将 capacity
